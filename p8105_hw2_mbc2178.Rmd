...
The `rightmrg_df` was created from a `right_join` between `snp` and `pols`. The number of 
observations are `r nrow(rightmrg_df)` and `r ncol(rightmrg_df)`. We have data from the 
following years `r range(rightmrg_df$year)`. 
...
merge and `unemply_df`. The number of observations are `r nrow(leftmrg_df)` and `r ncol(leftmrg_df)`. We have data from the 
following years `r range(leftmrg_df$year)`. 
This `dataset contains the following variables:
```{r}
str(leftmrg_df)
```
